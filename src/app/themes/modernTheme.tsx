    ...
    <div className='overflow-y-hidden w-full h-full flex items-center'>
      <div
        ...
        <div className='h-full flex-1 max-h-[360px] max-w-[640px]'>
          <div
            className={`${font.value} bg-white px-12 justify-center text-left rounded-2xl h-full p-4 flex flex-col`}>
            <h1 className={`text-5xl ${font?.lineHeight || 'leading-[1.2]'} font-bold text-gray-800 text-shadow-lg`}>{title}</h1>
            <h2 className='text-2xl mt-10 font-semibold text-left text-gray-800 text-shadow-sm'>{author}</h2>
          </div>
        </div>
      </div>
    </div>
